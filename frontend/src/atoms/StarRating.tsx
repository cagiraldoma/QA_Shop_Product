...
export const StarRating: React.FC<StarRatingProps> = ({
  ...
  return (
    <div className="flex items-center gap-0.5" role={interactive ? 'radiogroup' : undefined} aria-label={`Rating: ${rating} out of ${max}`}>
      {Array.from({ length: max }, (_, i) => {
        const value = i + 1;
        const filled = interactive ? value <= (hovered || rating) : value <= rating;
